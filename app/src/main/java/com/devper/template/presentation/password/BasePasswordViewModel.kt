...
open class BasePasswordViewModel(
    ...
    val newPassword = MutableLiveData("password")
    val confirmPassword = MutableLiveData("password")
    val password = MutableLiveData(SetPasswordParam())
    val passwordScore = password.switchMap { liveData { emit(it.password.getPasswordScore()) } }
